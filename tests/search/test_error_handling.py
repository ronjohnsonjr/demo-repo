"""Error handling tests for search endpoints.
...
def _mock_client(status_code: int) -> GitHubHTTPClient:
    ...
    mock_resp.headers = {}
    mock_resp.url = f"{BASE_URL}/search/repositories"
...
class TestSearchErrorHandling:
    """Error code tests for search endpoints."""

    def test_bad_request_400(self):
        """Verify 400 for malformed search queries.

        Setup: Mock client returning 400.
        Action: GET search endpoint.
        ...
        # Action
        resp = client.get(ENDPOINT_SEARCH_REPOS, params={"q": ""})

        # Assertions
        assert_status_code(resp, HTTP_400_BAD_REQUEST)

    def test_unauthorized_401(self):
        """Verify 401 for unauthenticated search on restricted endpoints.
        ...
        # Action
        resp = client.get("/search/repositories")

        # Assertions
        assert_status_code(resp, HTTP_401_UNAUTHORIZED)

    def test_forbidden_403(self):
        """Verify 403 for forbidden search requests (e.g., code search without token).

        Setup: Mock client returning 403.
        Action: GET code search endpoint.
        ...
        # Action
        resp = client.get("/search/code")

        # Assertions
        assert_status_code(resp, HTTP_403_FORBIDDEN)

    def test_unprocessable_422(self):
        """Verify 422 for invalid search query parameters.

        Setup: Mock client returning 422.
        Action: GET search endpoint with invalid params.
    ...
    def test_rate_limited_429(self):
        """Verify 429 when the search rate limit (30 req/min) is exceeded.
